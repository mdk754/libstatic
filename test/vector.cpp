...
TEST_CASE("Construct a vector", "[constructor]") {
	...
	SECTION("Value initialize") {
		...
		SECTION("From a range") {
			...
			sstd::vector<int, count> a(&array[0], &array[count]);
	...
	SECTION("Copy construct") {
		sstd::vector<unsigned char, 3> a(3, 8);

		SECTION("From an equivalent vector") {
			sstd::vector<unsigned char, 3> b(a);
		...
		SECTION("From a smaller vector") {
			sstd::vector<unsigned char, 5> b(a);
		...
		SECTION("From a larger vector") {
			sstd::vector<unsigned char, 2> b(a);
...
TEST_CASE("Assign a vector", "[assignment]") {
	sstd::vector<char, 3> a(3, 8);

	SECTION("From an equivalent vector") {
		sstd::vector<char, 3> b(1, 16);
...
TEST_CASE("Iterate over a vector", "[iterator]") {
	typedef sstd::vector<int, 3> vector;
	vector a(3, 4);

	SECTION("Forward direction") {
		for (vector::iterator it = a.begin(); it != a.end(); ++it) { *it = 16; }
	...
	SECTION("Reverse direction") {
		...
		for (vector::reverse_iterator it = a.rbegin(); it != a.rend();
		     ++it, ++i) {
			*it = i;
		}
...
TEST_CASE("Test vectors for equality", "[comparison]") {
	...
	SECTION("With a smaller size vector") {
		sstd::vector<char, 2> b(2, 16);

		REQUIRE(a != b);
	}

	SECTION("With a larger size vector") {
		sstd::vector<char, 5> b(5, 16);
...
TEST_CASE("Compare vectors lexicographically", "[comparison]") {
	sstd::vector<int, 4> a(4, 16);

	SECTION("With an equal size vector") {
		SECTION("With lower value") {
			sstd::vector<int, 4> b(4, 8);
	...
	SECTION("With a smaller size vector") {
		sstd::vector<int, 2> b(2, 16);

		REQUIRE(b < a);
	}

	SECTION("With a larger size vector") {
		sstd::vector<int, 8> b(8, 16);
